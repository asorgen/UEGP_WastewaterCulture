...
#Date: 01-18-21
#Description: Generate taxonomic relative abundance plots for upstream and downstream locations by antibiotic treatment.
...
dFrame <- dFrame[dFrame$Location %in% c("UPA","DSA"),]
...
dFrame$Location <- factor(dFrame$Location, levels = c("UPA","DSA"))
...
ggsave(plot, file=paste0(output, "UPA_DSA_antibiotics_absolute_abundance.pdf"),width = 20,height = 10)
...
ggsave(plot2, file=paste0(output, "UPA_DSA_antibiotics_relative_abundance.pdf"),width = 20,height = 10)
