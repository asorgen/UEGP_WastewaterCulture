...
inputModule = dir(pipeRoot, pattern="RelAbun_Condition_Location", full.names=TRUE)
input = file.path(inputModule,"output/")
output = file.path(moduleDir,"output/")

taxa <- "Filtered_Family"

dFrame=read.table(paste0(input, taxa, "_Abundance_by_Condition_and_Location.tsv"),sep='\t',header = TRUE)
dFrame <- dFrame[dFrame$Location %in% c("UPA","RES","HOS","INF","PCI","PCE","ATE","FCE","UV","DSA"),]
...
ab <- c("UPA","RES","HOS","INF","PCI","PCE","ATE","FCE","UV","DSA")
names(ab) <- c("UPA","RES","HOS","INF","PCI","PCE","ATE","FCE","UV","DSA")
...
plot2 <- ggplot(dFrame, aes(fill=Class_Family, y=Average, x=Location)) +
...
ggsave(plot2, file=paste0(output, "Condition_Location_relative_abundance.pdf"),width = 20,height = 15)
