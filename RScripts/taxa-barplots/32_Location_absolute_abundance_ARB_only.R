...
dFrame <- dFrame[dFrame$Antibiotic %in% c("Amp","Cip","Dox","Sulf"),]
...
ggsave(plot, file=paste0(output, taxa_input, "_absAbun_ARBonly_Location.pdf"),width = 20,height = 10)


UPA <- dFrame[dFrame$Location %in% c("UPA","UV","DSA") & dFrame$Class_Family %in% taxa,]
taxaPresent <- unique(UPA$Class_Family)
colors <- colorIDs[colorIDs$taxaNames %in% taxaPresent,]
palette <- colors$hexID

UPA$Location <- factor(UPA$Location, levels = c("UPA","UV","DSA"))
...
plot <- ggplot(UPA, aes(fill=Class_Family, y=Average, x=Location)) +
  ...
  scale_x_discrete(labels = c("UP","UV","DS"))+
  scale_y_continuous(labels = scientific)

ggsave(plot, file=paste0(output, taxa_input, "_absAbun_ARBonly_Location_inset.pdf"),width = 10,height = 5)
