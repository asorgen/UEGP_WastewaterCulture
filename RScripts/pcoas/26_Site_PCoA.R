...
#Date: 01-08-21
#Description: Generate PCoAs based on Bray-Curtis distances for OTUs classfied from the genus to phylum levels clustered by site.
...
for (g in 1:length(taxa_input)) {
  ...
  adon.results<-adonis(myT[, taxaStart:ncol(myT)] ~ myT$Site, method="bray",perm=999)
  # print(adon.results)
  capture.output(adon.results, file = paste0(outputDir,"_adonis_Site.txt"))
  Title <- paste0(taxa_input[g], " PERMANOVA p = ", adon.results$aov.tab$`Pr(>F)`[1])
  
  pdf(paste(outputDir, "_PCoA_Site_Normalized.pdf",sep = ""))
  ...
  legend("bottomright", c("Mallard","Sugar"),
...
adon.results<-adonis(myT[, taxaStart:ncol(myT)] ~ myT$Site, method="bray",perm=999)
# print(adon.results)
capture.output(adon.results, file = paste0(outputDir,"_adonis_Site_OTU.txt"))
Title <- paste0("OTU PERMANOVA p = ", adon.results$aov.tab$`Pr(>F)`[1])

pdf(paste(output, "OTU_PCoA_Site_Normalized.pdf",sep = ""))
par(mar=c(5, 5, 5, 5), xpd=TRUE, mfrow = c(1,1))
