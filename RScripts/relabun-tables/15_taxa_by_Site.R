...
#Date: 01-07-21
#Description: Generate relative abundance tables at each taxonomic level based on site.
...
for (z in 1:length(taxa)) {
  ...
  write.table(dFrame, file=paste0(output, taxa[z],"_Abundance_by_Site.tsv"),sep = "\t", row.names = FALSE)
...
for (z in 1:length(taxa)) {
  ...
  write.table(dFrame, file=paste0(output, taxa[z],"_ARB_Abundance_by_Site.tsv"),sep = "\t", row.names = FALSE)
