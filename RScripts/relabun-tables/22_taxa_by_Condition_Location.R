...
#Date: 01-18-21
#Description: Generate relative abundance tables at each taxonomic level based on incubation condition and location.
...
for (z in 1:length(taxa)) {
  ...
  for (x in 1:length(var1)) {
    ...
    var2=factor(unique(df$Location))
    
    for (y in 1:length(var2)) {
      Location=paste(var2[y])
      df2=df[df$Location %in% Location,]
      
      TAXA=factor(unique(frame$Taxa))
      Total_CFU=sum(df2$Raw.Norm)
      ...
      for (i in 1:length(TAXA)) {
        ...
        addition=data.frame(Condition,Location,Taxa,Taxa_Total,Total_CFU,Percentage,Average,SD)
  ...
  write.table(dFrame, file=paste0(output, taxa[z],"_Abundance_by_Condition_and_Location.tsv"),sep = "\t", row.names = FALSE)
}
