...
#Date: 01-07-21
#Description: Generate relative abundance tables at each taxonomic level based on incubation temperature.
...
for (z in 1:length(taxa)) {
  ...
  for (x in 1:length(list)) {
    Temperature=paste(list[x])
    df=frame[frame$Temperature %in% Temperature,]
    ...
    for (i in 1:length(TAXA)) {
      ...
      addition=data.frame(Temperature,Taxa,Taxa_Total,Total_CFU,Percentage,Average,SD)
  ...
  write.table(dFrame, file=paste0(output, taxa[z],"_Abundance_by_Temperature.tsv"),sep = "\t", row.names = FALSE)
...
for (z in 1:length(taxa)) {
  ...
  write.table(dFrame, file=paste0(output, taxa[z],"_ARB_Abundance_by_Temperature.tsv"),sep = "\t", row.names = FALSE)
