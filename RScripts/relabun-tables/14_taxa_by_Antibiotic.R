...
#Date: 01-07-21
#Description: Generate relative abundance tables at each taxonomic level based on antibiotic.
...
for (z in 1:length(taxa)) {
  ...
  list <- c("Neg", "Amp", "Cip", "Dox", "Sulf")
  
  ## Antibiotic averages
  dFrame=data.frame()
  
  for (x in 1:length(list)) {
    Antibiotic=paste(list[x])
    df=frame[frame$Antibiotic %in% Antibiotic,]
    ...
    for (i in 1:length(TAXA)) {
      ...
      addition=data.frame(Antibiotic,Taxa,Taxa_Total,Total_CFU,Percentage,Average,SD)
  ...
  write.table(dFrame, file=paste0(output, taxa[z],"_Abundance_by_Antibiotic.tsv"),sep = "\t", row.names = FALSE)
