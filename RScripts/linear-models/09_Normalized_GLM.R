...
for (z in 1:length(input)) {
  ...
  myT=read.table(paste0(inputDir, input[z],'_Normalized.tsv'),sep='\t',header = TRUE)
  ...
  write.table(dFrame, file=paste0(outputDir, input[z],"_GLM_Normalized_Combined.tsv"), sep="\t",row.names=FALSE)
  ...
  if (min(dFrame$pValuesSampleTypeAdjusted) < 0.05) {
    ...
    write.table(dFrame2, file=paste0(output, input[z],"_Significant_Taxa_ALL_SampleType.tsv"), sep="\t",row.names=FALSE)
  }
  ...
  if (min(dFrame$pValuesConditionAdjusted) < 0.05) {
    ...
    write.table(dFrame2, file=paste0(output, input[z],"_Significant_Taxa_ALL_Condition.tsv"), sep="\t",row.names=FALSE)
  }
  ...
  pdf(paste(PDFdir, input[z],"_Site_Normalized_Combined.pdf",sep = ""))
  ...
  pdf(paste(PDFdir, input[z],"_Location_Normalized_Combined.pdf",sep = ""))
  for (j in 1:nrow(dFrame)){
    absolute_index=dFrame$absolute_indexes[j]
    index=dFrame$indexes[j]
  ...
  pdf(paste(PDFdir, input[z],"_SampleType_Normalized_Combined.pdf",sep = ""))
  for (j in 1:nrow(dFrame)){
    absolute_index=dFrame$absolute_indexes[j]
    index=dFrame$indexes[j]
  ...
  pdf(paste(PDFdir, input[z],"_Condition_Normalized_Combined.pdf",sep = ""))
  for (j in 1:nrow(dFrame)){
    absolute_index=dFrame$absolute_indexes[j]
    index=dFrame$indexes[j]
  ...
  ########## PCoA for SampleType
  pdf(paste(PDFdir, input[z],"_PCoA_SampleType_Normalized_Combined.pdf",sep = ""))
  for (x in 1:5){
    for (y in 2:4){
      if (x==y){break}
  ...
  ########## PCoA for Site
  pdf(paste(PDFdir, input[z],"_PCoA_Site_Normalized_Combined.pdf",sep = ""))
  for (x in 1:5){
    for (y in 2:4){
      if (x==y){break}
      par(mar=c(5.1, 4.1, 4.1, 8.1), xpd=TRUE)
  ...
  ########## PCoA for Location
  pdf(paste(PDFdir, input[z],"_PCoA_Location_Normalized_Combined.pdf",sep = ""))
  for (x in 1:5){
    for (y in 2:4){
      if (x==y){break}
      par(mar=c(5.1, 4.1, 4.1, 8.1), xpd=TRUE)
  ...
  ########## PCoA for Condition
  pdf(paste(PDFdir, input[z],"_PCoA_Condition_Normalized_Combined.pdf",sep = ""))
  for (x in 1:5){
    for (y in 2:4){
      if (x==y){break}
      par(mar=c(5.1, 4.1, 4.1, 8.1), xpd=TRUE)
